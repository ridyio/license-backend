import { Controller, ForbiddenException, Get, NotFoundException, Query, Req, UnprocessableEntityException } from '@nestjs/common';
...
export class ClientController {
    ...
    @Get('verify')
    async verify(@Query('purchaseCode') purchaseCode: string, @Query('port') usedPort: string, @Req() request: Request): Promise<VerifyResponse> {
        ...
        if (purchase === undefined) {
            ...
            const application = await this.applicationService.getByItemId(license.item.id);
        ...
        if (enabledClients < purchase.licenseCount) {
            const token = purchase.clients.length < 1 ? shortid.generate() : purchase.clients[0].token;
            await this.clientService.verifyNewClient(purchase, ip as string, token, port);
            return {
                status: 'OK',
                token: token
            };
    ...
    @Get('disable_one')
    async disableOne(@Query('ip') ip: string,@Query('purchaseCode') purchaseCode: string) {
        let purchase = null;
